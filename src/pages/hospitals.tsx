...
export default function Hospitals() {
  ...
  async function fetchHospitals(lat?: number, lng?: number, km: number = radius) {
    ...
    if (lat !== undefined && lng !== undefined) {
      response = await supabase.rpc("nearby_hospitals", {
        lat: lat,
        lng: lng,
        radius_meters: km * 1000,
      });
    } else {
      response = await supabase.from("hospitals").select("*");
    }
  ...
  }, [search, ownershipFilter, lgaFilter, specialtyFilter, radius, setSearchParams]);
  ...
  const filteredHospitals = hospitals.filter((h) => {
    ...
    const matchesOwnership = !ownershipFilter || h.ownership_type === ownershipFilter;
    const matchesLga = !lgaFilter || h.lga === lgaFilter;
    const matchesSpecialty = !specialtyFilter || (h.specialties && h.specialties.includes(specialtyFilter));
  ...
  function exportHospitals() {
    ...
    link.href = url;
    link.download = `hospitals-${search || "all"}-${new Date().toISOString().split("T")[0]}.csv`;
    link.click();
  }
    ...
    <div className="flex flex-col" style={{ height: "100svh", overflow: "hidden" }}>
      ...
      >
        ...
        <input
          ...
          className="pl-4 pr-4 py-1.5 rounded-lg text-xs outline-none"
          style={{ background: "white", border: "1px solid var(--border)", color: "black" }}
        />
      ...
      {locationError && (
        ...
        >
          ...
          <button
            onClick={() => { setLocationError(null); setUserLocation(null); }}
        ...
        <main className="hidden sm:block flex-1 relative" style={{ background: "var(--code-bg)" }}>
          ...
          {selectedHospital && (
            ...
            >
              ...
              <h2 className="mb-3 leading-snug" style={{ color: "var(--text-h)", fontSize: "16px" }}>
